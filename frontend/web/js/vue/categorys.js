...
new Vue({
    ...
    computed: {
        ...
        product() {
            if (!this.isSelectedProduct || !this.products)
                return null
        ...
        landOrientDetail: function () {
            if (this.isEmptyValue("land_orient"))
                return "-"
        ...
        paperSizeDetail: function () {
            if (this.isEmptyValue("paper_size_id"))
                return "-"
            const data = this.findDataOption(
        ...
        bookBindingDetail: function () {
            const book_binding_status = this.formAttributes.book_binding_status
            const book_binding_id = this.formAttributes.book_binding_id
            let text = ''
            if(book_binding_status && book_binding_status === '1'){
                text += 'เข้าเล่ม '
            } else if(book_binding_status && book_binding_status === '0'){
                text += 'ไม่เข้าเล่ม '
            }
            if(book_binding_id){
                const item = this.BookBindingOpts.data.find(item => item.id === book_binding_id)
                if(item){
        ...
        perforateDetail: function () {
            if (this.isEmptyValue("perforate"))
                return "-"
            const option = this.findDataOption(
        ...
        perforatedRippedDetail: function () {
            if (this.isEmptyValue("perforated_ripped"))
                return "-"
        ...
        runningNumberDetail: function () {
            if (this.isEmptyValue("running_number"))
              return "-"
            const running_number = this.findDataOption(this.runningNumberOptions, "value", "running_number")
            return this.getTextValue(running_number)
        },

        windowBoxDetail: function () {
            if (!this.showWindowBoxInput)
                return "ไม่ติดหน้าต่างกล่อง"
        ...
        coatingDetail: function () {
            if (this.isEmptyValue("coating_id"))
                return "-"
        ...
        dicutDetail: function () {
            if (!this.isDicut && this.radioChecked === "not-dicut")
                return "ไม่ไดคัท"
        ...
        book_binding_qty: function () {
            if (!this.formAttributes.book_binding_qty)
                return "-"
            return this.formAttributes.book_binding_qty
        },
        page_qty: function () {
            if (!this.formAttributes.page_qty)
                return "-"
    ...
    methods: {
        ...
        async fetchDataOptions() {
            ...
            try {
                ...
                // หน่วยติดหน้าต่างกล่อง
                this.windowBoxUnitOpts = await updateObject(this.windowBoxUnitOpts, {
                    data: this.mapDataOptions(dataOptions.windowBoxUnitOpts)
                })
        ...
        onChangewindowBox: function (e) {
            this.formAttributes.window_box = e.target.value
            if (e.target.value === "1") {
                this.showWindowBoxInput = true;
            } else {
                this.formAttributes.window_box_length = ""
                this.formAttributes.window_box_unit = null
                this.formAttributes.window_box_width = ""
